...
export default function ApplicationDetailPage() {
  ...
  const [application, setApplication] =
    useState<StudentPlacementEnquiryDetail | null>(null);
  ...
  useEffect(() => {
    const fetchApplication = async () => {
      ...
      const result = await getStudentPlacementEnquiryById(
        params.id as string,
        token,
      );
      if (result.success) {
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "success" || paymentStatus === "completed") {
      ...
      if (token && params.id) {
        getStudentPlacementEnquiryById(params.id as string, token).then(
          (result) => {
            if (result.success) {
              setApplication(result.data);
    ...
    <div>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-[#687179]">
                ID: {application._id}
              </p>
            ...
            <div className="sm:text-right">
              ...
              <span
                className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${stageStyles[application.stage] ?? ""}`}
              >
                {application.stage}
              </span>
            </div>
              ...
              {PROGRESS_STEPS.map((step, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p
                      className={`mt-3 text-sm font-medium ${done ? "text-brand-navy" : "text-[#3d454b]"}`}
                    >
                      {step.label}
                    </p>
                    ...
                    {active && (
                      <span
                        className={`mt-1 text-[10px] px-2 py-0.5 rounded-full font-medium ${studentStatusStyles[application.studentStatus] ?? ""}`}
                      >
          ...
          <div className="flex flex-wrap gap-3 border-t border-brand-border pt-4">
            <div>
              ...
              <span
                className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${paymentStyles[application.firstPayment] ?? ""}`}
              >
            ...
            <div>
              ...
              <span
                className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${paymentStyles[application.finalPayment] ?? ""}`}
              >
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl bg-white p-4 shadow-soft sm:p-6 lg:col-span-2">
          <div className="mb-5 flex flex-col items-center border-b border-brand-border pb-5 text-center sm:mb-6 sm:pb-6">
            ...
            <h2 className="font-display font-bold text-brand-navy text-lg">
              {application.studentData?.fullName ??
          ...
          <div className="flex flex-col gap-4">
            {[
              { icon: Mail, label: "Email", value: application.email },
              {
                icon: GraduationCap,
                label: "University",
                value: application.universityOrMedicalSchool,
              },
              ...
              { icon: Globe, label: "Language", value: application.language },
              {
                ...
                value: application.preferredCities,
              },
          ...
          <div className="mt-5 flex flex-wrap gap-3 border-t border-brand-border pt-4">
            <div>
              ...
              <span
                className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${genericStatusStyles[application.hospitalStatus] ?? ""}`}
              >
            ...
            <div>
              ...
              <span
                className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${genericStatusStyles[application.adminStatus] ?? ""}`}
              >
        ...
        <div className="flex flex-col gap-4 sm:gap-5 lg:col-span-3">
          <div className="rounded-2xl bg-white p-4 shadow-soft sm:p-6">
            <div className="mb-4 flex flex-col items-start gap-3 sm:flex-row sm:justify-between">
              ...
              <div className="text-left sm:text-right">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-medium ${studentStatusStyles[application.studentStatus] ?? ""}`}
                >
                  {application.studentStatus}
                </span>
              </div>
          ...
          {/* Submitted documents */}
          <div className="rounded-2xl bg-white p-4 shadow-soft sm:p-6">
            <h3 className="font-semibold text-brand-navy text-sm mb-4">
              Submitted Documents
            </h3>
                ...
                >
                  <div className="flex min-w-0 items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-brand-navy text-xs font-medium">
                        Document {index + 1}
                      </p>
      ...
      {showPaymentButton(
        ...
        >
          <button className="w-full rounded-xl bg-brand-teal px-8 py-3 text-sm font-medium text-white transition-all hover:bg-brand-tealDark sm:w-auto">
            {application.firstPayment === "pending"
              ? "Pay First Payment"
              : "Pay Final Payment"}
          </button>
        </div>
